...
const doDrag = (event: MouseEvent) => {
  if (isDragging.value && isZoomedIn.value && event.buttons === 1) {
    performDrag(event.clientX, event.clientY, event.currentTarget as HTMLElement)
    event.preventDefault()
...
const doTouch = (event: TouchEvent) => {
  if (isDragging.value && isZoomedIn.value && event.touches.length === 1) {
    const touch = event.touches[0]
    performDrag(touch.clientX, touch.clientY, event.currentTarget as HTMLElement)
    event.preventDefault()
  }
}
...
const performDrag = (clientX: number, clientY: number, container: HTMLElement) => {
